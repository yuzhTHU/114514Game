...
from ..env import Jmcomic, Symbol


class GP:
    """ Genetic Programming """
    def __init__(self, env:Jmcomic):
        self.env = env
    ...
    def solve(self, n_episode=1000, early_stop=False, quiet=False):
        ...
        for episode in range(1, n_episode+1):
            ...
            self.population = sorted(self.population, key=lambda x: self.env.reward(x), reverse=True)
            self.population = self.population[:self.n_population]

            best = self.env.reward(self.population[0])
            if best > self.best[1]: 
                self.best = (self.population[0], best)

            if early_stop and self.population[0].eval():
                break
